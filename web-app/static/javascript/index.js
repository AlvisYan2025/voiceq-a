function get_problem_sets(){
    fetch('/ps').then((res)=>res.json()).then(data=>{
        ...
        const user_ps = data['user'];
        const user_saved = data['saved'];
        if (!user_ps || !user_ps){
            return 
        }
        user_ps.forEach(problem_set => {
            //create a new container to display each problem set
            const cardContainer = document.createElement('div');
            cardContainer.className = 'col-md-4';
            const card = document.createElement('div');
            ...
            // Append the card to the card container
            cardContainer.appendChild(card);
            const card_list = document.getElementById("my-ps-list");
            card_list.appendChild(cardContainer);
        });
        user_saved.forEach(problem_set => {
            //create a new container to display each problem set
            const cardContainer = document.createElement('div');
            cardContainer.className = 'col-md-4';
            const card = document.createElement('div');
            ...
            // Append the card to the card container
            cardContainer.appendChild(card);
            const card_list = document.getElementById("saved-ps-list");
            card_list.appendChild(cardContainer);
        });
    })
